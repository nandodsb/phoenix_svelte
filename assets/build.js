	...
	setup(build) {
		...
		build.onLoad({ filter: /.*/, namespace: 'svonix-ns' }, async () => {
			const svelte_files = glob.sync('js/svelte/**/*.svelte', {
				ignore: 'js/svelte/**/_*.svelte',
			})
			const svelte_filenames = svelte_files.map((el) =>
				path.relative('js/svelte', el).replace('.svelte', ''),
			)
